...
/**
 * Tabla de solo lectura: toda la edición (nombre, DNI, celular, grupo, base, sector)
 * pasa por DeviceEditModal — ya no hay edición inline por doble clic en la celda.
 */
export const DevicesTable: React.FC<DevicesTableProps> = ({
  ...
  const renderCell = (device: ManagedDevice, col: DeviceColumnDef) => {
    if (col.key === 'name') {
      return (
    ...
    if (col.key === 'uniqueId') {
      return <span className="text-[13px] font-mono text-zinc-600 dark:text-zinc-400">{device.uniqueId}</span>;
    }
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-white dark:bg-zinc-900 overflow-hidden relative">
            ...
            {cargando ? (
              ...
              devices.map((device) => {
                ...
                return (
                  <tr
                    key={device.id}
                    onDoubleClick={() => onOpenEditModal(device)}
      ...
      {selectedIds.size > 0 && (
        <div className="absolute bottom-14 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2.5 px-3.5 py-1.5 bg-zinc-900/95 dark:bg-zinc-100/95 text-white dark:text-zinc-900 rounded-2xl shadow-2xl backdrop-blur-xl border border-zinc-700/50 animate-in fade-in slide-in-from-bottom-2 duration-200">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1 bg-[#155BD0] hover:bg-[#114eb3] text-white rounded-lg text-[12px] font-medium shadow-xs transition-colors cursor-pointer"
          >
            <Layers size={12} />
          ...
          <button
            ...
            className="text-[12px] text-zinc-400 hover:text-white dark:hover:text-zinc-900 underline transition-colors cursor-pointer"
          >
            Desmarcar
          </button>
        </div>
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-t border-zinc-200/80 dark:border-zinc-800 bg-zinc-50/70 dark:bg-zinc-900/60 text-[12px] shrink-0 select-none flex-wrap gap-2">
        <div className="flex items-center gap-2.5 text-zinc-500 dark:text-zinc-400 flex-wrap">
          ...
          <select
            ...
            className="h-7 px-2.5 bg-white dark:bg-zinc-800 border border-zinc-200/80 dark:border-zinc-700 rounded-lg text-zinc-700 dark:text-zinc-300 font-medium text-[12px] focus:outline-none cursor-pointer shadow-2xs"
          >
          ...
          <span className="text-zinc-400 text-[12px]">Total: {totalRegistros} dispositivos</span>
        </div>

        <div className="flex items-center gap-2.5">
          <span className="text-zinc-500 dark:text-zinc-400 font-mono text-[12px] font-medium">
