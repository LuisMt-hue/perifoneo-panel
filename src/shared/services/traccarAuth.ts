...
/**
 * Constantes de duración de sesiones y tokens en Traccar
 */
export const SESSION_TOKEN_EXPIRATION_DAYS = 30;
export const MS_PER_DAY = 24 * 60 * 60 * 1000;
export const SESSION_TOKEN_EXPIRATION_MS = SESSION_TOKEN_EXPIRATION_DAYS * MS_PER_DAY;
...
export async function loginTraccar(credenciales: LoginCredenciales): Promise<LoginResultado> {
  ...
  let sessionToken = '';
  try {
    const expira = new Date(Date.now() + SESSION_TOKEN_EXPIRATION_MS).toISOString();
    const tokenRes = await fetch('/api/session/token', {
      ...
      body: new URLSearchParams({ expiration: expira }).toString(),
      credentials: 'include',
    });
